...
const Clubs = ({ navigation }) => {
    ...
    const handleJoinedClub = () => {
        resetNavigation(navigation, SCREENS.JOINED_CLUB)
    }

    useFocusEffect(
        useCallback(() => {
            dispatch(getAllClubs({ page, limit: 10, searchPayload: { user_id: user_id, search: searchTerm } }));
    ...
    const onRefresh = () => {
        setRefreshing(true);
        setPage(1)
        dispatch(getAllClubs({ page: 1, limit: 10, searchPayload: { user_id: user_id, search: searchTerm } }))
    ...
    return (
        <SafeAreaView style={styles.container}>
            <Header
                ...
                thirdIconName={"group"}
                thirdIconColor={theme.colors.labelColors}
                onThirdIconPress={handleJoinedClub}
            />
